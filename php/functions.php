...
function show_files(/*$part_type, $part_id*/) {
  ...
  $HTMLstr = "Hello";
  if(!file_exists($dir)) {
  	$HTMLstr = "<p>No files found</p><br>";
  } else {
    if(file_exists($dir) && ($handle = opendir($dir))) {
      while(false !== ($entry=readdir($handle))) {
        if($entry != "." && $entry != "..") {
          $HTMLstr = $HTMLstr + "<a href=\"$dir/$entry\" target=\"_blank\">$entry</a><br>";
        }
      }
    }
  }
  ...
  if(!file_exists($dir)) {
    ...
    while(false !== ($entry=(readdir($handle)))) {
      if($entry != "." && $entry != ".." && substr($entry,-3) !="txt") {
        ...
        $HTMLstr += "<tr><td>";
        $HTMLstr += "<a href=$dir/$str target=\"blank\"> <img src=\"$dir/$entry\" width=\"200\" height=\"200\"></a>";
        $HTMLstr += "</td><td>";
        $txt = $dir."/".substr($entry,0,-3)."txt";
        if(file_exists($txt)) {
          $fp = fopen($txt, 'r');
          $HTMLstr += nl2br(fread($fp, filesize($txt)));
          fclose($fp);
        }
...
function addTempVsTimeFile($type, $id, $file) {
  $directory = "../phase_2_prod/files/$type/$id/";
...
function addSheetThicknessContour($id, $file) {
  $directory = "../phase_2_prod/files/sheet/$id/";
  if (!file_exists($directory)) {
    mkdir($directory);
    chmod($directory, 0777);
  }
...
function add_file($type,$id,$files) {
  $targetDir = "../../phase_2_prod/files/$type/$id/";
  ...
  for ($i = 0; $i < sizeof($files['name']); $i++) {
    ...
    if (!move_uploaded_file($files['tmp_name'][$i], $targetFile)) { // If file fails to upload send error message
      echo "<br><h2>Sorry, an error has occurred.</h2><br>";
    }
  }
}
