<?php
  ...
  $db = new Database();
  $sql = "INSERT INTO wafer (name) VALUES (\"".$_POST['name']."\")";
  $db->query($sql);
  $sql = "SELECT MAX(id) FROM wafer";
  $db->query($sql);
  $db->singleRecord();
  $id = $db->Record['MAX(id)'];
  $sql = "INSERT INTO notes (part_type,part_id) VALUES (\"wafer\",$id)";
  $db->query($sql);
  ...
  $sql = "UPDATE wafer SET lastEdit=\"$time\" WHERE id=$id";
  $db->query($sql);
